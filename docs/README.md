...
| [crd-reference.md](crd-reference.md) | All seven `frame.plume-labs.io/v1alpha1` CRDs — fields, controllers, webhooks |
...
- **Operator** (`internal/`) — Kubebuilder v4 controllers for six of the seven CRDs. This is the layer that actually mutates cluster state. FrameUser is the exception: it is a record `authd` reads, not something reconciled.
