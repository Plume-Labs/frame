...
- [ ] Get `make lint` back to green. It reports 21 issues — deprecated `GetEventRecorderFor`, unchecked `c.Close`, dead `corev1GVK`, repeated string literals. None are new; all of them predate this phase and none sit in code changed here. A permanently red lint target trains everyone to ignore it.
...
- [ ] Close the SDK's coverage gap. `frame-sdk.ts` reads and writes four kinds — FrameNode, FrameJob, SchedulingPolicy, FrameResourceQuota. **TalosMachineConfig and TalosUpgrade appear nowhere in `src/`**, so the two CRDs that drive node OS configuration and upgrades have no control-plane surface at all. V1's definition of done claims the UI and SDK drive real CRDs; today they drive four sevenths of them.
- [ ] Install the FrameUser CRD on the test cluster. `frameusers.frame.plume-labs.io` is absent there while `cluster-control-auth` has been running for eight days — the deployed operator predates the type, so every FrameUser read or write returns NotFound. authd is more inert than "deployed but unused": it has no store.
