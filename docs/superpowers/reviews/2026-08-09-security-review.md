...
> **Follow-up, 2026-08-09: the `status` half was done, and it is not enough.** The API freeze moved the field to `status.passwordHash` (F11) and gave `FrameUser` the three RBAC tiers it never had, with no `frameusers/status` rule on viewer or editor. Measured against a real apiserver afterwards: that buys **write** protection only. A viewer with `get frameusers` and no status rule at all still reads the hash, because a plain `GET` returns the whole object including `status` — a status subresource splits writes, not reads. The write half is genuine: an editor's merge patch carrying spec *and* status applies the spec half and the apiserver drops the status half silently, where in `spec` the same patch overwrote the hash.
>
> So this finding's "or at minimum to `status`" understates what the Secret is for. **The Secret is the only fix for the read side**, and until it lands the operational rule is that **`get frameusers` is equivalent to holding every password hash**. Nothing is exposed today — zero FrameUsers exist and no tier is bound — but `frameuser-viewer-role` becomes a credential-disclosure role the moment authd Stage 2 makes the tiers meaningful. That deserves a date, not a comment.
