...
type FrameUserSpec struct {
	...
	// guess about. No MinLength is needed — "" is not in the enum — and the
	// kubebuilder Required marker would not have helped, since it does not
	// reject an empty string from a typed client.
	// +optional
	// +kubebuilder:validation:Enum=enabled;disabled
...
// the v1alpha1 FrameUser for why the two could not be separated.
//
