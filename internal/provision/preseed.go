...
// comment names. A machine that reaches `netcfg-done` has proven only that
// it could still reach provisiond after the re-run -- there is no `set -e`
// here and BeaconSend ends in `|| true`, so a re-run that did NOT take the
// static address still executes this line if the machine can still reach
// the listener some other way (e.g. a residual DHCP lease it had moments
// earlier, when it sent `netcfg`). The signal is in the absence, not the
// presence: a machine that lost reachability after the re-run never sends
// `netcfg-done` at all. One that reaches the guard and is refused there
// will have sent both `netcfg` and `netcfg-done`, but never `early`.
...
#
# The terminator after the then-branch is emitted BY the inner conditional,
# not hardcoded after it, because the two branches end differently: when a
# beacon is configured, the branch ends in BeaconHeartbeat's own trailing
# "&", which is already a shell separator (separator_op : '&' | ';') and
# must not be followed by another one -- "...&; else" is a syntax error, sh
# -n exits 2, and the whole early_command silently fails to run at all,
# guard included. With no beacon, the branch is a bare ":" and does need an
# explicit ";" before "else". Hardcoding a semicolon after the inner
# conditional's own close got this wrong for the first case; the fix is
# that each branch supplies its own correct terminator instead.
